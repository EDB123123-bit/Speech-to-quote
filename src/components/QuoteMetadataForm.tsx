...
export default function QuoteMetadataForm({ quote }: { quote: Quote }) {
  ...
  return <form action={action} className="quote-sidebar-card flex flex-col gap-3">
    ...
    <label className="label flex flex-col gap-1">Offertenummer<input className="field" name="quote_number" required readOnly={quote.quote_kind === 'meerwerk'} defaultValue={quote.quote_number ?? quote.id.split('-')[0].toUpperCase()} /></label>
